...
public class OtherPersonButterScreen : UIWindow
{
    [Header("参观模式")]
    [SerializeField] private Button exitVisitBtn; // 退出参观按钮 (需要在Prefab中拖拽赋值)
    ...
    {
        exitVisitBtn.AddClickAction(OnExitVisitClick);
    ...
    {
        exitVisitBtn.interactable = state;
    }
    /// <summary>
    /// 背景及蝴蝶飞行初始化
    /// </summary>
    private IEnumerator BackgroundInit()
    {
        ...
        int targetGardenId = GameCoreManager.Instance.otherPersonProfile.current_garden_id;
        
        GameObject go = AdvancedBundleLoader.SharedInstance.LoadGameObject("butterflybg", "ButterflyBg"+ targetGardenId);
        if(go == null)
            yield break;
    ...
    /// 退出参观按钮事件
    /// </summary>
    private void OnExitVisitClick()
    {
        // 隐藏当前界面，并返回到记录的来源界面
        SystemManager.Instance.HidePanel(PanelType.OtherPersonButterScreen);
    }
    ...
    private IEnumerator FlaySelectButterfly()
    {
        ...
        int targetGardenId = GameCoreManager.Instance.otherPersonProfile.current_garden_id;
        List<ButterflyInfo> allInfos = ButterfliesManager.Instance.GetGardenButterflies(targetGardenId);
        List<int> ownedButterflyIds = new List<int>();
        if (GameCoreManager.Instance.otherPersonProfile.collected_butterflies != null)
        {
            // 将 DTO 中的字符串 ID 转换为 int
            foreach (string strId in GameCoreManager.Instance.otherPersonProfile.collected_butterflies)
            {
                if (int.TryParse(strId, out int id))
                {
                    ownedButterflyIds.Add(id);
                }
            }
        }
       
        // 这里改成乱序，并始终有最近获得的蝴蝶存在
        List<ButterflyInfo> ownedInfos = allInfos.FindAll(p=> ownedButterflyIds.Contains(p.Id));
