...
public class ChessDynamicHardSave
{
    /// <summary>
    /// 是否开启
    /// </summary>
    ...
    public string Getfilepath
    {
        get
        {
            return Path.Combine(Application.persistentDataPath, "ChessDynamicHardSave.json");
        }
    }
    ...
    public void InitData(ChessDynamicHardSave dynamicHardSave)
    {
        IsEnergy = dynamicHardSave.IsEnergy;
        ...
        IsDecrA = dynamicHardSave.IsDecrA;
        ReduceWord = dynamicHardSave.ReduceWord;
        // 复制关卡通关时长字典
        LevelClearTimes.Clear();
    ...
    /// <param name="decrA">是否减A了</param>
    public void UpdateEnergy(float value, bool decrA)
    {
        EnergyValue += value;
        if(decrA)
            this.IsDecrA = !decrA;
    ...
    /// </summary>
    /// <param name="value"></param>
    public void UpdateMaxEnergyLevel(int value)
    {
        if (value > MaxEnergyLevel)
        {
            IsDecrA = true;
            ReduceWord = 0;
        }
    ...
    {           
        ...
        try
        {
            if (File.Exists(filePath))
            {
                ...
                {
                    ChessDynamicHardSave dynamicHardSave = JsonConvert.DeserializeObject<ChessDynamicHardSave>(json);               
    ...
    {
        if (string.IsNullOrEmpty(GameDataManager.Instance.UserData.logoutTime)) return;

        DateTime desTime = DateTime.Parse(GameDataManager.Instance.UserData.logoutTime);
