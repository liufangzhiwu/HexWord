namespace Water2D
{
	...
	public class Water2DEditor : Editor {
        ...
        SerializedProperty ParticlesTag;
        SerializedProperty GlowEffect;
        SerializedProperty GlowColor;
        SerializedProperty GlowSize;
        SerializedProperty GlowSortingOrder;
        SerializedProperty _lastGlowEnabledValue;
        SerializedProperty _glowTex;
        ...
        internal void OnEnable(){
            ...
            ParticlesTag = serializedObject.FindProperty("ParticlesTag");
            ...
            WaterStyle = serializedObject.FindProperty("Water2DType");
            ...
            GlowSize = serializedObject.FindProperty("GlowSize");
            GlowSortingOrder = serializedObject.FindProperty("GlowSortingOrder");
            _lastGlowEnabledValue = serializedObject.FindProperty("_lastGlowEnabledValue");
            _glowTex = serializedObject.FindProperty("_glowTex");
            ...
            {
                if (UnityEditorInternal.InternalEditorUtility.tags[i].Contains(ParticlesTag.stringValue))
                {
                    _selectedTag = i;
                    break;
                }
        ...
        public override void OnInspectorGUI () {
            ...
            textSubHeaderStyle3.normal.textColor = new Color(.51f, .51f, .51f);
            ...
            string subText = WSpawner.Water2DType.ToString();

            // POPUP WaterStyle mode
        
            Water2D_Spawner.EnumTypes tipes = (Water2D_Spawner.EnumTypes)WaterStyle.enumValueIndex;

            EditorGUILayout.BeginHorizontal();
            tipes = (Water2D_Spawner.EnumTypes)EditorGUILayout.EnumPopup("",tipes, EditorStyles.miniButton);
            if (WaterStyle.enumValueIndex != (int)tipes)
                WaterStyle.enumValueIndex = (int)tipes;
            ...
            */
            ...
            if (_material != null)
            {
                ...
                */
                ...
                //if(WaterStyle != null && Selection.transforms.Length < 2)
                //    WaterStyle.boolValue = EditorGUILayout.Toggle("Toon Style", WaterStyle.boolValue);


                if (WSpawner.Water2DType == Water2D_Spawner.EnumTypes.Regular) // Regular SHADER
                {
                    if (Selection.transforms.Length < 2)
                    {
                        Blending.boolValue = EditorGUILayout.Toggle("Blending Colors", Blending.boolValue);
                        if (Blending.boolValue)
                        {
                            EditorGUILayout.HelpBox("Allows mixing the color of different particles", MessageType.None);
                            EditorGUILayout.Space();
                        }
                    }

                    EditorGUILayout.PropertyField(FillColor, new GUIContent("Fill Color"));
                    EditorGUILayout.PropertyField(StrokeColor, new GUIContent("Fresnel Color"));
                ...
                if (WSpawner.Water2DType == Water2D_Spawner.EnumTypes.Toon) // TOON SHADER Obsolete
                {
                    EditorGUILayout.PropertyField(FillColor, new GUIContent("Fill Color"));
                    EditorGUILayout.PropertyField(StrokeColor, new GUIContent("Stroke Color"));
                    EditorGUILayout.Slider(AlphaCutOff, 0f, 1f, new GUIContent("Alpha CutOff"));
                    EditorGUILayout.Slider(AlphaStroke, 0f, 2f, new GUIContent("Alpha Stroke"));
                }
                else if (WSpawner.Water2DType == Water2D_Spawner.EnumTypes.Refracting) // REFRACTING SHADER
                {
                    Blending.boolValue = EditorGUILayout.Toggle("Blending Colors", Blending.boolValue);
                    ...
                    EditorGUILayout.PropertyField(StrokeColor, new GUIContent("Fresnel Color"));
                    EditorGUILayout.Slider(Distortion, 0f, 6f, new GUIContent("Distortion"));
                    EditorGUILayout.Slider(DistortionSpeed, 0f, 10f, new GUIContent("Speed"));
                ...
                _selectedTag = EditorGUILayout.Popup("Tag", _selectedTag, UnityEditorInternal.InternalEditorUtility.tags);
                ParticlesTag.stringValue = UnityEditorInternal.InternalEditorUtility.tags[_selectedTag];

                
                if (!GlowEffect.boolValue)
                {
                    GlowEffect.boolValue = EditorGUILayout.Toggle("Glow Effect", GlowEffect.boolValue);
                }
                else
                {
                    EditorGUILayout.BeginVertical("Box");
                    GlowEffect.boolValue = EditorGUILayout.Toggle("Glow Effect", GlowEffect.boolValue);
                    EditorGUILayout.PropertyField(GlowColor, new GUIContent("Color"));
                    EditorGUILayout.Slider(GlowSize, 1f, 3f, new GUIContent("Size"));
                    EditorGUILayout.PropertyField(GlowSortingOrder, new GUIContent("Sorting order"));
                    
                    EditorGUILayout.EndVertical();
                }

                if (_lastGlowEnabledValue.boolValue != GlowEffect.boolValue)
                {
                    _lastGlowEnabledValue.boolValue = GlowEffect.boolValue;
                    _needRebuild = true;
                }
            }
            ...
            if (foldoutEmission && WSpawner.Water2DEmissionType == Water2D_Spawner.EmissionType.ParticleSystem)
            {
                ...
                EditorGUILayout.Slider(DelayBetweenParticles, 0.00000001f, .1f, new GUIContent("Delay by particles"));
            ...
            if (foldoutEvents)
            {
                ...
                for (int i = 0; i < ShapeFillCollider2D.arraySize; i++)
                {
                    EditorGUILayout.PropertyField(ShapeFillCollider2D.GetArrayElementAtIndex(i), new GUIContent("Collider 2D (" + i.ToString() + ")"));
                }
